...
/*
 * Klasa odpowiedzialna za tworzenie customowych list - Generator Custom
 */
...
class CustomBuilder extends Builder
{
	...
	public function build_list()
	{
		...
		foreach ($this->attribs as $k => $v)
		{
			...
			if ($custom)
			{
				$custom_fields[] = $this->columns[$k]['db_name'];
			}
		}
		...
		if ($this->dates)
		{
			$main_text .= '<form style="display: inline-flex; margin: 0 0 0 2em;" action="index.php?route=' . MODULE_NAME . '&sort=' . $this->params['sort_column'] . '&order=' . $this->params['sort_order'] . '" class="navbar-form" role="search" method="post">';
			...
			$main_text .= '<button type="submit" id="SetDatesButton" name="SetDatesButton" class="btn btn-success">OK</button>';
			$main_text .= '</form>';
		}
		$main_text .= '<form style="display: inline-flex; margin: 0 0 0 2em;" action="index.php?route=' . MODULE_NAME . '&sort=' . $this->params['sort_column'] . '&order=' . $this->params['sort_order'] . '" class="navbar-form" role="search" method="post">';
		...
		foreach ($this->data as $k => $row)
		{
			...
			{
				if (in_array($key, $active_fields)) // normal
				{
				...
				if (in_array($key, $custom_fields)) // custom
				{
					if (isset($this->attribs[$idx]['custom']))
					{
						$custom_text = $value;
						$custom = $idx;
					}
