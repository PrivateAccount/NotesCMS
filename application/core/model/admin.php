		...
		foreach ($this->table_params as $k => $v)
		{
			...
			switch ($table)
			{
				case 'users':
					...
					break;
				case 'user_roles':
					$condition .= $_SESSION['user_status'] == USER ? ' AND user_roles.user_id = ' . $_SESSION['user_id'] : NULL;
					break;
				case 'categories':
					$condition .= $_SESSION['user_status'] == USER ? ' AND categories.author_id = ' . $_SESSION['user_id'] : NULL;
					break;
				case 'pages':
				...
				case 'comments':
					...
					break;
				case 'notes':
					$condition .= $_SESSION['user_status'] == USER ? ' AND notes.author_id = ' . $_SESSION['user_id'] : NULL;
					break;
				default:
					break;
			}
