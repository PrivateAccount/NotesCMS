...
class Script_Controller extends Controller
{
	public function __construct($app)
	{
		...
		$this->app->get_page()->set_path(array(
			...
			'index.php?route='.MODULE_NAME => 'Skrypt',
		));	
	...
	public function Reset_Action()
	{
		if ($this->app->get_acl()->allowed(ADMIN)) // są uprawnienia
		{
			...
			if (isset($_GET['confirm']))
			{
				...
				if ($result) $this->app->get_page()->set_message(MSG_INFORMATION, 'Skrypt został pomyślnie zresetowany.');
				else $this->app->get_page()->set_message(MSG_ERROR, 'Skrypt nie został zresetowany.');
